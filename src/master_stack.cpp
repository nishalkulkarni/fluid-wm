...
int MasterStack::stackSize() { return stack.size(); }

bool MasterStack::empty() { return stack.size() == 0; }

void MasterStack::insert(Window w) { stack.push_back(w); }
...
void MasterStack::swapMaster(Window stack_window) {
  std::vector<Window>::iterator posCurrent =
      find(stack.begin(), stack.end(), stack[0]);
  std::vector<Window>::iterator posStack =
      find(stack.begin(), stack.end(), stack_window);
...
Window MasterStack::getMaster() { return stack[0]; }
